...
router.post('/resetpassword/:token', async (req, res) => {
    ...
    const currentTime = new Date();
    if (req.session.resetTokenExpire < currentTime) {
        req.flash('message', 'El enlace de recuperación ha expirado.');
        return res.redirect('/forgotpassword');
    }
    ...
    try {
        ...
        if (!email) {
            req.flash('message', 'No se ha proporcionado un correo electrónico.');
            return res.redirect(`/resetpassword/${req.params.token}`);
        }
        ...
        if (result.affectedRows === 0) {
            req.flash('message', 'No se pudo actualizar la contraseña, por favor intenta nuevamente.');
            return res.redirect(`/resetpassword/${req.params.token}`);
        }
